...
func TestGetAccountInfoRequest(t *testing.T) {
	...
	proxyEth := ProxyETHGetCode{revoClient}
...
func TestGetCodeInvalidAddressRequest(t *testing.T) {
	...
	//prepare responses
	getAccountInfoErrorResponse := revo.GetErrorResponse(revo.ErrInvalidAddress)
	if getAccountInfoErrorResponse == nil {
		panic("mocked error response is nil")
	}
	err = mockedClientDoer.AddError(revo.MethodGetAccountInfo, getAccountInfoErrorResponse)
	if err != nil {
		t.Fatal(err)
	}
